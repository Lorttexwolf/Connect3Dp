...
namespace Lib3Dp.Plugins.OME
{
	public class OMEPlugin : IPlugin<OMEPlugin, OMEPluginConnectorData>
	{
		...
		private readonly Dictionary<string, MachineConnection> StreamsToMachine;
		...
		public bool RegisterConnector(MachineConnection connector)
		{
			StreamsToMachine[MakeStreamName(connector)] = connector;
			return true;
		}

		public bool IsConnectorRegistered(MachineConnection connector)
		{
			return StreamsToMachine.ContainsValue(connector);
		}

		public OMEPluginConnectorData GetConnectorPluginData(MachineConnection connector)
		{
